...
for epoch in range(niter):
    ...
    def compute_loss():
        ...
        loss = torch.empty(1, requires_grad=True)
        ...
        for n in range(len(timesteps)):
            ...
            for j in range(len(x)):
                ...
                output = model(torch.tensor([x[j], t], dtype=torch.float, requires_grad=True))
            ...
            shifted_y = torch.zeros(len(new_x), requires_grad=True)
